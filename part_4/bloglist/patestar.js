...
const mostBlogs = (blogs) => {
    ...
    blogs.reduce((acc, curr) => {
        ...
        if (acc[author]) {
            acc[author] = { author: author, blogs: acc[author].blogs + 1 }
        } else {
            acc[author] = { author: author, blogs: 1 }
        }
        return acc
    }, output)

    const objBlogs = Object.values(output)

    let authorWithMostBlogs = objBlogs.reduce((max, author) => max.blogs > author.blogs ? max : author)
